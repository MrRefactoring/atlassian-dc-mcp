...
describe('JiraService', () => {
  ...
  describe('application roles', () => {
    ...
    describe('updateApplicationRole', () => {
      ...
      it('revokes the application when both group lists are empty', async () => {
        const result = await jiraService.updateApplicationRole('jira-software', [], []);

        expect(result.success).toBe(true);
        expect(jira.request).toHaveBeenCalledWith(expect.objectContaining({
          body: { ...storedRole, groups: [], defaultGroups: [] },
        }));
      });

      it('serializes updates to the same application role', async () => {
        const roleAfterFirstUpdate = {
          ...storedRole,
          groups: ['jira-administrators'],
          defaultGroups: ['jira-administrators'],
        };
        let resolveFirstWrite!: (role: typeof storedRole) => void;
        const firstWrite = new Promise<typeof storedRole>((resolve) => {
          resolveFirstWrite = resolve;
        });

        (jira.admin.get as Mock)
          .mockResolvedValueOnce(storedRole)
          .mockResolvedValueOnce(roleAfterFirstUpdate);
        (jira.request as Mock)
          .mockImplementationOnce(() => firstWrite)
          .mockResolvedValueOnce({ ...roleAfterFirstUpdate, selectedByDefault: false });

        const firstUpdate = jiraService.updateApplicationRole(
          'jira-software',
          ['jira-administrators'],
          ['jira-administrators'],
        );
        const secondUpdate = jiraService.updateApplicationRole('jira-software', undefined, undefined, false);

        await vi.waitFor(() => expect(jira.request).toHaveBeenCalledTimes(1));
        expect(jira.admin.get).toHaveBeenCalledTimes(1);

        resolveFirstWrite(roleAfterFirstUpdate);
        const [firstResult, secondResult] = await Promise.all([firstUpdate, secondUpdate]);

        expect(firstResult.success).toBe(true);
        expect(secondResult.success).toBe(true);
        expect(jira.admin.get).toHaveBeenCalledTimes(2);
        expect(jira.request).toHaveBeenNthCalledWith(2, {
          method: 'PUT',
          url: '/api/2/applicationrole/jira-software',
          body: { ...roleAfterFirstUpdate, selectedByDefault: false },
        });
      });
