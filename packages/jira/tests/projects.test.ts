...
describe('JiraService', () => {
  ...
  describe('getProject', () => {
    it('gets a single project by id or key', async () => {
      ...
      (jira.projects.getProject as Mock).mockResolvedValue(mockProject);

      const result = await jiraService.getProject('TEST');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockProject);
      expect(jira.projects.getProject).toHaveBeenCalledWith({ projectIdOrKey: 'TEST' });
    });

    it('handles project not found errors', async () => {
      (jira.projects.getProject as Mock).mockRejectedValue(new Error('Project not found'));
  ...
  describe('createProject', () => {
    it('creates a project', async () => {
      ...
      expect(jira.projects.createProject).toHaveBeenCalledWith({ key: 'TEST', name: 'Test Project' });
    });
  ...
  describe('updateProject', () => {
    it('updates a project', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data).toBe(mockProject);
      expect(jira.projects.updateProject).toHaveBeenCalledWith({ projectIdOrKey: 'TEST', name: 'Renamed Project' });
    });
    ...
    it('forwards expand', async () => {
      ...
      expect(jira.projects.updateProject).toHaveBeenCalledWith({ projectIdOrKey: 'TEST', description: 'New description', expand: 'lead' });
    });
  ...
  describe('project entity properties', () => {
    it('gets project property keys', async () => {
      ...
      (jira.projects.getProjectPropertyKeys as Mock).mockResolvedValue(mockKeys);

      const result = await jiraService.getProjectPropertyKeys('TEST');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockKeys);
      expect(jira.projects.getProjectPropertyKeys).toHaveBeenCalledWith({ projectIdOrKey: 'TEST' });
    });

    it('handles errors getting project property keys', async () => {
      (jira.projects.getProjectPropertyKeys as Mock).mockRejectedValue(new Error('The project does not exist'));
    ...
    it('gets a project property', async () => {
      ...
      (jira.projects.getProjectProperty as Mock).mockResolvedValue(mockProperty);

      const result = await jiraService.getProjectProperty('TEST', 'my-property');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockProperty);
      expect(jira.projects.getProjectProperty).toHaveBeenCalledWith({ propertyKey: 'my-property', projectIdOrKey: 'TEST' });
    });

    it('handles errors getting a project property', async () => {
      (jira.projects.getProjectProperty as Mock).mockRejectedValue(new Error('The property with given key is not found'));
    ...
    it('sets a project property', async () => {
      (jira.projects.setProjectProperty as Mock).mockResolvedValue(undefined);

      const result = await jiraService.setProjectProperty('TEST', 'my-property', '{"a":1}');

      expect(result.success).toBe(true);
      expect(jira.projects.setProjectProperty).toHaveBeenCalledWith({ propertyKey: 'my-property', projectIdOrKey: 'TEST', body: { key: 'my-property', value: '{"a":1}' } });
    });

    it('handles errors setting a project property', async () => {
      (jira.projects.setProjectProperty as Mock).mockRejectedValue(new Error('The calling user does not have permission to administer the project'));
    ...
    it('deletes a project property', async () => {
      (jira.projects.deleteProjectProperty as Mock).mockResolvedValue(undefined);

      const result = await jiraService.deleteProjectProperty('TEST', 'my-property');

      expect(result.success).toBe(true);
      expect(jira.projects.deleteProjectProperty).toHaveBeenCalledWith({ propertyKey: 'my-property', projectIdOrKey: 'TEST' });
    });

    it('handles errors deleting a project property', async () => {
      (jira.projects.deleteProjectProperty as Mock).mockRejectedValue(new Error('The property with given key is not found'));
  ...
  describe('components', () => {
    it('creates a component', async () => {
      ...
      (jira.projectComponents.createComponent as Mock).mockResolvedValue(mockComponent);

      const result = await jiraService.createComponent('TEST', 'Backend');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockComponent);
      expect(jira.projectComponents.createComponent).toHaveBeenCalledWith({ project: 'TEST',
        name: 'Backend',
    ...
    it('gets paginated components', async () => {
      ...
      (jira.projectComponents.getPaginatedComponents as Mock).mockResolvedValue(mockPage);

      const result = await jiraService.getComponents();

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockPage);
      expect(jira.projectComponents.getPaginatedComponents).toHaveBeenCalledWith({});
    });

    it('gets a single component', async () => {
      const mockComponent = { id: '10000', name: 'Backend' };
      (jira.projectComponents.getComponent as Mock).mockResolvedValue(mockComponent);

      const result = await jiraService.getComponent('10000');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockComponent);
      expect(jira.projectComponents.getComponent).toHaveBeenCalledWith({ id: '10000' });
    });

    it('updates a component', async () => {
      const mockComponent = { id: '10000', name: 'Backend v2' };
      (jira.projectComponents.updateComponent as Mock).mockResolvedValue(mockComponent);

      const result = await jiraService.updateComponent('10000', 'Backend v2');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockComponent);
      expect(jira.projectComponents.updateComponent).toHaveBeenCalledWith({ id: '10000', body: {
        name: 'Backend v2',
        description: undefined,
        leadUserName: undefined,
      } });
    ...
    it('deletes a component', async () => {
      (jira.projectComponents.deleteComponent as Mock).mockResolvedValue(undefined);

      const result = await jiraService.deleteComponent('10000');

      expect(result.success).toBe(true);
      expect(jira.projectComponents.deleteComponent).toHaveBeenCalledWith({ id: '10000' });
    });

    it('gets component related issue counts', async () => {
      const mockCounts = { issueCount: 5 };
      (jira.projectComponents.getComponentRelatedIssues as Mock).mockResolvedValue(mockCounts);
  ...
  describe('versions', () => {
    it('creates a version', async () => {
      ...
      (jira.projectVersions.createVersion as Mock).mockResolvedValue(mockVersion);

      const result = await jiraService.createVersion('TEST', '1.0');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockVersion);
      expect(jira.projectVersions.createVersion).toHaveBeenCalledWith({ project: 'TEST',
        name: '1.0',
        description: undefined,
    ...
    it('gets paginated versions with defaults', async () => {
      ...
      (jira.projectVersions.getPaginatedVersions as Mock).mockResolvedValue(mockPage);

      const result = await jiraService.getVersions();

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockPage);
      expect(jira.projectVersions.getPaginatedVersions).toHaveBeenCalledWith({ maxResults: 100, query: '' });
    });

    it('gets a single version', async () => {
      const mockVersion = { id: '10000', name: '1.0' };
      (jira.projectVersions.getVersion as Mock).mockResolvedValue(mockVersion);

      const result = await jiraService.getVersion('10000');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockVersion);
      expect(jira.projectVersions.getVersion).toHaveBeenCalledWith({ id: '10000' });
    });

    it('updates a version', async () => {
      (jira.projectVersions.updateVersion as Mock).mockResolvedValue({ id: '10000', released: true });

      const result = await jiraService.updateVersion('10000', undefined, undefined, true);

      expect(result.success).toBe(true);
      expect(jira.projectVersions.updateVersion).toHaveBeenCalledWith({ id: '10000', body: {
        name: undefined,
        description: undefined,
        released: true,
        archived: undefined,
    ...
    it('deletes and replaces a version', async () => {
      (jira.projectVersions.deleteVersionAndSwap as Mock).mockResolvedValue(undefined);

      const result = await jiraService.deleteAndReplaceVersion('10000', 10001, 10002);

      expect(result.success).toBe(true);
      expect(jira.projectVersions.deleteVersionAndSwap).toHaveBeenCalledWith({ id: '10000', moveFixIssuesTo: 10001,
        moveAffectedIssuesTo: 10002 });
    });

    it('merges a version into another', async () => {
      (jira.projectVersions.merge as Mock).mockResolvedValue(undefined);

      const result = await jiraService.mergeVersion('10000', '10001');

      expect(result.success).toBe(true);
      expect(jira.projectVersions.merge).toHaveBeenCalledWith({ moveIssuesTo: '10001', id: '10000' });
    });

    it('moves a version', async () => {
      const mockVersion = { id: '10000' };
      (jira.projectVersions.moveVersion as Mock).mockResolvedValue(mockVersion);

      const result = await jiraService.moveVersion('10000', 'First');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockVersion);
      expect(jira.projectVersions.moveVersion).toHaveBeenCalledWith({ id: '10000', position: 'First', after: undefined });
    });

    it('gets version related issue counts', async () => {
      const mockCounts = { issuesFixedCount: 3 };
      (jira.projectVersions.getVersionRelatedIssues as Mock).mockResolvedValue(mockCounts);
    ...
    it('gets version unresolved issue counts', async () => {
      ...
      (jira.projectVersions.getVersionUnresolvedIssues as Mock).mockResolvedValue(mockCounts);
    ...
    it('handles errors', async () => {
      (jira.projectVersions.getVersion as Mock).mockRejectedValue(new Error('Version does not exist'));
  ...
  describe('project roles', () => {
    ...
    it('replaces all actors for a role', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data).toBe(mockRole);
      expect(jira.projects.setActors).toHaveBeenCalledWith({ projectIdOrKey: 'TEST', id: 10000, body: {
    ...
    it('adds actors to a role', async () => {
      ...
      expect(result.success).toBe(true);
      expect(jira.projects.addActorUsers).toHaveBeenCalledWith({ projectIdOrKey: 'TEST', id: 10000, user: ['john.doe'],
        group: ['admins'] });
    });
  ...
  describe('project categories', () => {
    it('gets all project categories', async () => {
      ...
      (jira.projectCategories.getAllProjectCategories as Mock).mockResolvedValue(mockCategories);

      const result = await jiraService.getProjectCategories();

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockCategories);
      expect(jira.projectCategories.getAllProjectCategories).toHaveBeenCalledWith();
    });

    it('creates a project category', async () => {
      const mockCategory = { id: '2', name: 'New Category' };
      (jira.projectCategories.createProjectCategory as Mock).mockResolvedValue(mockCategory);

      const result = await jiraService.createProjectCategory('New Category', 'A category');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockCategory);
      expect(jira.projectCategories.createProjectCategory).toHaveBeenCalledWith({ name: 'New Category',
        description: 'A category' });
    });

    it('gets a project category by id', async () => {
      const mockCategory = { id: '1', name: 'Category 1' };
      (jira.projectCategories.getProjectCategoryById as Mock).mockResolvedValue(mockCategory);

      const result = await jiraService.getProjectCategory(1);

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockCategory);
      expect(jira.projectCategories.getProjectCategoryById).toHaveBeenCalledWith({ id: 1 });
    });

    it('updates a project category', async () => {
      const mockCategory = { id: '1', name: 'Renamed' };
      (jira.projectCategories.updateProjectCategory as Mock).mockResolvedValue(mockCategory);

      const result = await jiraService.updateProjectCategory(1, 'Renamed');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockCategory);
      expect(jira.projectCategories.updateProjectCategory).toHaveBeenCalledWith({ id: 1, body: {
        name: 'Renamed',
        description: undefined,
      } });
    });
    ...
    it('deletes a project category', async () => {
      (jira.projectCategories.removeProjectCategory as Mock).mockResolvedValue(undefined);

      const result = await jiraService.deleteProjectCategory(1);

      expect(result.success).toBe(true);
      expect(jira.projectCategories.removeProjectCategory).toHaveBeenCalledWith({ id: 1 });
    });

    it('handles errors', async () => {
      (jira.projectCategories.getProjectCategoryById as Mock).mockRejectedValue(new Error('The category does not exist'));
  ...
  describe('validateProjectKey', () => {
    it('returns an empty error collection for a valid key', async () => {
      ...
      (jira.projectKeyAndNameValidation.validateProjectKey as Mock).mockResolvedValue(mockValidation);

      const result = await jiraService.validateProjectKey('TEST');

      expect(result.success).toBe(true);
      expect(result.data).toBe(mockValidation);
      expect(jira.projectKeyAndNameValidation.validateProjectKey).toHaveBeenCalledWith({ key: 'TEST' });
    });
    ...
    it('handles errors', async () => {
      (jira.projectKeyAndNameValidation.validateProjectKey as Mock).mockRejectedValue(new Error('Not authenticated'));
