...
import { handleApiOperation } from '../src/api-error-handler.js';
...
describe('handleApiOperation', () => {
  ...
  it('formats API client errors with the prefix and exposes the body as details', async () => {
    const operation = vi.fn().mockRejectedValue(apiError(404, 'Not Found', { message: 'missing' }));
  ...
  it('does not retry 4xx client errors (not transient)', async () => {
    const operation = vi.fn().mockRejectedValue(apiError(400, 'Bad Request'));
  ...
  it('gives up after maxRetries and returns the final formatted error', async () => {
    const operation = vi.fn().mockRejectedValue(apiError(503, 'Service Unavailable', { retry: false }));
  ...
  it('does not retry at all when maxRetries is 0, even for a retryable status', async () => {
    const operation = vi.fn().mockRejectedValue(apiError(429, 'Too Many Requests'));
